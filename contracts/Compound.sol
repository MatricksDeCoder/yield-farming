...
// contract to interact with Compound
contract Compound {

    // references to the Compotroller 
    ...
    /// @notice function to claim COMP for participating
    function claimComp() internal {
        comptroller.claimComp(address(this));
    }

    /// @notice function to COMP Address
    /// @return the COMP address
    function getCompAddress() internal view returns(address) {
        return comptroller.getCompAddress();
    }

    /// @notice function to get cToken Balance
    /// @return the amount of cToken this address has
    ...
    /// @dev  view keyword because borrowBalanceCurrent() can be called in a tx, and Solidity complains if view
    /// @notice function to get the botrow Balance and interest
